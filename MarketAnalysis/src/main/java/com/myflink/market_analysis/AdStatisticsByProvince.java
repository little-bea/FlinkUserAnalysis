...
import com.myflink.market_analysis.bean.AdCountViewByProvince;
import com.myflink.market_analysis.bean.BlackListUserWarning;
...
public class AdStatisticsByProvince {
    ...
    public static class FilterBlackListUser extends KeyedProcessFunction<Tuple, AdClickEvent, AdClickEvent> {
        ...
        public void open(Configuration parameters) throws Exception {
            ...
            isSendState = getRuntimeContext().getState(new ValueStateDescriptor<Boolean>("is-sent", Boolean.class, false));
        }

        @Override
        public void processElement(AdClickEvent adClickEvent, Context ctx, Collector<AdClickEvent> collector) throws Exception {
            // 判断当前用户对同一广告的点击次数， 如果不够上限， 就count加1正常输出：如果达到上限，直接过滤掉，并测输出流输出黑名单报警
            // 首先获取当前得到count值
